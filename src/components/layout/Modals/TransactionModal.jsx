...
const TransactionModal = ({ product, onClose }) => {
  ...
  useEffect(() => {
    const fetchCard = async () => {
      try {
        const response = await axios.get(`${API}/api/cards/get-active-card/${userId}`);
        setCard(response.data);
        setCardData({
          cardNumber: `****  ****  ****  ${response.data.last4Digits}`,
          expiration: response.data.expiryDate,
          brand: response.data.bankName,
        });
      } catch (error) {
  ...
  return (
    <>
      ...
      {showConfirmation && <PurchaseConfirmationModal isOpen={showConfirmation} onClose={onClose} />}
    </>
  );
};
